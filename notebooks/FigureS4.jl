...
md"
# Figure S4. Moisture Budget from WRF
"
...
begin
	...
	for istn = 1 : 12
		...
		ds  = NCDataset(datadir("wrf","processed","$(geo.ID)-QBUDGET-20190801_20201231.nc"))
		p  = ds["P"][:]
		e  = ds["E"][:]
		∇  = ds["∇"][:] * 3600
		Δ  = ds["ΔWVP"][:]
...
begin
	...
	for istn = 1 : 12, ibox = 1 : 4
		...
		ds  = NCDataset(datadir("wrf","processed","$(geo.ID)-∇decompose-20190801_20201231.nc"))
		ds  = NCDataset(datadir("wrf","processed","$(geo.ID)-QBUDGET-20190801_20201231.nc"))
		p  = smooth(ds["P"][:],days=7)
		e  = smooth(ds["E"][:],days=7)
		∇  = smooth(ds["∇"][:],days=7) * 3600
	...
	open(datadir("wrf","qbudgetdiff-smooth07.txt"),"w") do io
			writedlm(io,Δmat)
	end
end
...
begin
	...
	open(datadir("wrf","qbudgetdiff-smooth30.txt"),"w") do io
		writedlm(io,Δmat)
	end
end
