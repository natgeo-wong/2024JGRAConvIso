...
begin
	nds = read(npd,geo,Date(yr,mo))
	nprcp = nds["precipitation"][:,:,mo] * 86400
	close(nds)
end
...
# ╔═╡ 0858e700-9a6f-430a-a431-75ee4a97916d
nnprcp = regrid(wprcp,nlsd);

# ╔═╡ 330e270e-4203-4bfc-be20-98f3d9ae89fa
neprcp = regrid(wprcp,elsd);
...
begin
	...
	c1 = axs[1].pcolormesh(nlsd.lon,nlsd.lat,nprcp',levels=2:2:30,cmap="Blues",extend="both")
	axs[2].pcolormesh(nlsd.lon,nlsd.lat,nnprcp',levels=2:2:30,cmap="Blues",extend="both")
	c2 = axs[3].pcolormesh(nlsd.lon,nlsd.lat,(nnprcp.-nprcp)',levels=vcat(-5:-1,-0.5,0.5,1:5)*4,cmap="drywet",extend="both")
	
	axs[4].pcolormesh(elsd.lon,elsd.lat,eprcp',levels=2:2:30,cmap="Blues",extend="both")
	axs[5].pcolormesh(elsd.lon,elsd.lat,neprcp',levels=2:2:30,cmap="Blues",extend="both")
	axs[6].pcolormesh(elsd.lon,elsd.lat,(neprcp.-eprcp)',levels=vcat(-5:-1,-0.5,0.5,1:5)*4,cmap="drywet",extend="both")
	...
	for ax in axs
		...
		ax.format(
			...
			suptitle = "Mean Rainfall Rate ($yr $(monthname(mo)))"
		)
	end
	...
	fig.savefig(projectdir("figures","figS2b-imergvswrfvsera-$(Dates.format(Date(yr,mo),dateformat"yyyymm")).png"),transparent=false,dpi=400)
	load(projectdir("figures","figS2b-imergvswrfvsera-$(Dates.format(Date(yr,mo),dateformat"yyyymm")).png"))
end
