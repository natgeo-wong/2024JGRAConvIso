...
md"
# Figure S7. Station W-Weighted Pressure, $\sigma$, from WRF Modelling, w/ Advection
"
		...
		if (wvc[ibox+1] < 0.15) .& (qvl[ibox+1] < 0.05)
			...
			for it = 1 : nt
				if ((prcp[it]+advc[it]-evap[it])>2.5) && !isnan(pwgt[it])
					...
					binstn[rind,pind] += (hvyp[it] + hvya[it] - hvye[it]) * 0.25
					prcstn[rind,pind] += (prcp[it] + advc[it] - evap[it]) * 0.25
				end
			end
		end
	end
...
begin
	...
	f1.savefig(projectdir("figures","figS7-wrfadvec.png"),transparent=false,dpi=400)
	load(projectdir("figures","figS7-wrfadvec.png"))
end
